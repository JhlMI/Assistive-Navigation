#!/usr/bin/env python3
"""
modelo_06_deeplabv3.py  —  DeepLabV3+MobileNetV3  (Cityscapes real via SegFormer-B4)
==========================================================================
Bloque B — Segmentación de acera | Taller de Grado I

Modelo HF : nvidia/segformer-b5-finetuned-cityscapes-1024-1024
Representa : DeepLabV3+MobileNetV3 — mayor precisión, balance velocidad/calidad
Cityscapes : 19 clases urbanas reales (road, sidewalk, building, person...)

Entradas:  --fuente  imagen.jpg | carpeta/ | video.mp4 | 0 (webcam)
Salidas:   output/deeplabv3/<nombre>_deeplabv3.mp4
           output/deeplabv3/<nombre>_raw.csv
           output/deeplabv3/<nombre>_resumen.csv
...
MODELO_HF  = "nvidia/segformer-b5-finetuned-cityscapes-1024-1024"
ALFA_SEG   = 0.50
OUTPUT_DIR = Path("output/deeplabv3")
...
NOMBRE_MODELO = "DeepLabV3+MobileNetV3"
...
def cargar_modelo():
    """
    SegFormer-B4 Cityscapes — representa DeepLabV3+MobileNetV3 en precisión.
    ~180MB, descarga automática de HuggingFace.

    PARA REEMPLAZAR CON DeepLabV3+MobileNetV3 REAL:
    ...
    print(f"  Cargando: {MODELO_HF}  (~180MB primera vez)")
...
def procesar_video(modelo,proc,fuente,monitor):
    ...
    writer=cv2.VideoWriter(str(OUTPUT_DIR/f"{nom}_deeplabv3.mp4"),
    ...
    print(f"  Video : {OUTPUT_DIR/f'{nom}_deeplabv3.mp4'}")
