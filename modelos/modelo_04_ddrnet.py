#!/usr/bin/env python3
"""
modelo_04_ddrnet.py  —  DDRNet-23-slim  (Cityscapes real via SegFormer-B2)
==========================================================================
Bloque B — Segmentación de acera | Taller de Grado I

Modelo HF : nvidia/segformer-b3-finetuned-cityscapes-1024-1024
Representa : DDRNet-23-slim — mayor precisión, balance velocidad/calidad
Cityscapes : 19 clases urbanas reales (road, sidewalk, building, person...)

Entradas:  --fuente  imagen.jpg | carpeta/ | video.mp4 | 0 (webcam)
Salidas:   output/ddrnet/<nombre>_ddrnet.mp4
           output/ddrnet/<nombre>_raw.csv
           output/ddrnet/<nombre>_resumen.csv
...
MODELO_HF  = "nvidia/segformer-b3-finetuned-cityscapes-1024-1024"
ALFA_SEG   = 0.50
OUTPUT_DIR = Path("output/ddrnet")
...
def cargar_modelo():
    """
    SegFormer-B2 Cityscapes — representa DDRNet-23-slim en precisión.
    ~80MB, descarga automática de HuggingFace.

    PARA REEMPLAZAR CON DDRNet-23-slim REAL:
    ...
    print(f"  Cargando: {MODELO_HF}  (~80MB primera vez)")
...
def procesar_video(modelo,proc,fuente,monitor):
    ...
    writer=cv2.VideoWriter(str(OUTPUT_DIR/f"{nom}_ddrnet.mp4"),
    ...
    print(f"  Video : {OUTPUT_DIR/f'{nom}_ddrnet.mp4'}")
