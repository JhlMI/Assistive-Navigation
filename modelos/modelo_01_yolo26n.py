#!/usr/bin/env python3
"""
modelo_01_yolo26n.py  —  YOLO26n  (detección pura, NMS-free)
=============================================================
Bloque A — Detección de obstáculos | Taller de Grado I

Entradas
--------
  --fuente   imagen.jpg | carpeta/ | video.mp4 | 0 (webcam)

Salidas
-------
  output/yolo26n/<nombre>_yolo26n.mp4   video anotado
  output/yolo26n/<nombre>_raw.csv       datos por frame
  output/yolo26n/<nombre>_resumen.csv   estadísticas agregadas para el informe

Métricas en pantalla
--------------------
  FPS · Latencia · Obstáculos · RAM · GPU mem · GPU% · CPU%
...
OUTPUT_DIR = Path("output/yolo26n")
...
COLORES = {
    ...
    1:  (0,   165, 255),  # bicycle
    2:  (255, 120, 0  ),  # car
    3:  (0,   200, 0  ),  # motorcycle
    5:  (200, 0,   180),  # bus
    7:  (0,   200, 255),  # truck
    9:  (0,   230, 230),  # traffic light
    11: (180, 0,   255),  # stop sign
    56: (255, 80,  80 ),  # chair
    57: (80,  255, 80 ),  # couch
    60: (80,  80,  255),  # dining table
    39: (0,   180, 180),  # bottle
    24: (80,  200, 120),  # backpack
}
...
class Monitor:
    def __init__(self):
        ...
        if PSUTIL_OK:
            psutil.cpu_percent()  # calentar (primera llamada siempre es 0)

        # GPU via pynvml (igual que Task Manager)
        if NVML_OK:
...
def dibujar(frame, resultado, lat_ms, recursos, frame_id, monitor):
    ...
    # Panel derecho — recursos del sistema
    der = [
        ("SISTEMA",                                          (0,220,255)),
    ...
    # Pie — dispositivo y resolución
    dev = monitor.gpu_nombre
    _txt(vis, f"{dev}  |  {w}x{h}  |  conf={CONFIANZA}",
...
def imprimir_resumen(registros, nombre):
    ...
    print(f"\n  {'─'*50}")
    print(f"  RESUMEN — YOLO26n  |  {nombre}")
    print(f"  {'─'*50}")
...
def procesar_imagen(modelo, ruta, monitor):
    ...
    cv2.imshow("YOLO26n", vis); cv2.waitKey(0); cv2.destroyAllWindows()
...
def procesar_video(modelo, fuente, monitor):
    ...
    delay=1   # sin delay — corre tan rápido como la GPU permita
    es_wc=isinstance(fuente,int)
    nom=("webcam" if es_wc else Path(str(fuente)).stem)
    writer=cv2.VideoWriter(str(OUTPUT_DIR/f"{nom}_yolo26n.mp4"),
        ...
        if not pausado:
            ...
            if n%30==0:
                ...
                print(f"  Frame {n:>5}  lat={lat_ms:5.1f}ms  FPS={fps_r:.1f}"
                      f"  det={n_det}  RAM={rec['ram_gb']:.1f}GB  VRAM={rec['gpu_mb']:.0f}MB  GPU={rec['gpu_pct']:.0f}%")
            cv2.imshow("YOLO26n",vis)
    ...
    print(f"  Video : {OUTPUT_DIR/f'{nom}_yolo26n.mp4'}")
...
def main():
    ...
    parser=argparse.ArgumentParser(description="YOLO26n")
    ...
    print(f"\n{'─'*52}")
    print(f"  YOLO26n  —  Bloque A  |  Taller de Grado I")
