#!/usr/bin/env python3
"""
modelo_02_yolo26n_seg.py  —  YOLO26n-seg  (detección + segmentación de instancias)
====================================================================================
Bloque A — Detección de obstáculos | Taller de Grado I

Entradas:  --fuente  imagen.jpg | carpeta/ | video.mp4 | 0 (webcam)
Salidas:   output/yolo26n_seg/<nombre>_yolo26n_seg.mp4
           output/yolo26n_seg/<nombre>_raw.csv
           output/yolo26n_seg/<nombre>_resumen.csv
...
OUTPUT_DIR = Path("output/yolo26n_seg")
OUTPUT_DIR.mkdir(parents=True, exist_ok=True)

PALETA = [
    (0,   0,   255), (0,   165, 255), (0,   255, 255), (0,   255, 0  ),
    (255, 0,   0  ), (255, 0,   128), (128, 0,   255), (255, 255, 0  ),
    (0,   128, 255), (128, 255, 0  ), (255, 128, 0  ), (0,   255, 128),
    (64,  0,   255), (255, 64,  0  ), (0,   64,  255), (64,  255, 0  ),
    (200, 0,   200), (0,   200, 200), (200, 200, 0  ), (100, 100, 255),
]
...
def dibujar(frame, resultado, lat_ms, recursos, frame_id, monitor):
    ...
    if cajas is not None:
        ...
        for i, caja in enumerate(cajas):
            ...
            if mascos is not None and i < len(mascos):
                mask_raw = mascos.data[i].cpu().numpy()
                mask_bin = (cv2.resize(mask_raw,(w,h)) > 0.5).astype(np.uint8)
                overlay  = vis.copy()
                overlay[mask_bin == 1] = color
                vis = cv2.addWeighted(vis, 1-ALFA_MASK, overlay, ALFA_MASK, 0)
                conts,_ = cv2.findContours(mask_bin, cv2.RETR_EXTERNAL,
                                           cv2.CHAIN_APPROX_SIMPLE)
                cv2.drawContours(vis, conts, -1, color, 1)
    ...
    _txt(vis, f"{monitor.gpu_nombre}  |  {w}x{h}  |  conf={CONFIANZA}  alfa={ALFA_MASK}",
...
def guardar_resumen(registros, ruta, info_sys, fuente_nombre):
    ...
    with open(ruta,"w",newline="",encoding="utf-8") as f:
        w = csv.writer(f)
        w.writerow(["# RESUMEN YOLO26n-seg"])
...
def imprimir_resumen(registros, nombre):
    ...
    print(f"\n  {'─'*50}")
    print(f"  RESUMEN — YOLO26n-seg  |  {nombre}")
    print(f"  {'─'*50}")
...
def procesar_imagen(modelo, ruta, monitor):
    ...
    cv2.imshow("YOLO26n-seg", vis); cv2.waitKey(0); cv2.destroyAllWindows()
...
def procesar_video(modelo, fuente, monitor):
    ...
    writer=cv2.VideoWriter(str(OUTPUT_DIR/f"{nom}_yolo26n_seg.mp4"),
    ...
    print(f"  Video : {OUTPUT_DIR/f'{nom}_yolo26n_seg.mp4'}")


# ── Main ──────────────────────────────────────────────────────────────────────

def main():
    global CONFIANZA, IOU, IMGSZ, ALFA_MASK
    parser=argparse.ArgumentParser(description="YOLO26n-seg")
    ...
    parser.add_argument("--alfa",  type=float, default=ALFA_MASK,
                        help="Opacidad de máscaras (0.0-1.0)")
    args=parser.parse_args()
    CONFIANZA,IOU,IMGSZ,ALFA_MASK=args.conf,args.iou,args.imgsz,args.alfa

    print(f"\n{'─'*52}")
    print(f"  YOLO26n-seg  —  Bloque A  |  Taller de Grado I")
    print(f"  conf={CONFIANZA}  iou={IOU}  imgsz={IMGSZ}  alfa={ALFA_MASK}")
    print(f"{'─'*52}")
