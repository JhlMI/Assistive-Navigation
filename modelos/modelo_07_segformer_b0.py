#!/usr/bin/env python3
"""
modelo_04_ddrnet.py  —  SegFormer-B0  (Cityscapes real via SegFormer-B2)
==========================================================================
Bloque B — Segmentación de acera | Taller de Grado I

Modelo HF : nvidia/segformer-b0-finetuned-cityscapes-1024-1024
Representa : SegFormer-B0 — mayor precisión, balance velocidad/calidad
Cityscapes : 19 clases urbanas reales (road, sidewalk, building, person...)

Entradas:  --fuente  imagen.jpg | carpeta/ | video.mp4 | 0 (webcam)
Salidas:   output/segformer_b0/<nombre>_segformer_b0.mp4
           output/segformer_b0/<nombre>_raw.csv
           output/segformer_b0/<nombre>_resumen.csv
...
MODELO_HF  = "nvidia/segformer-b0-finetuned-cityscapes-1024-1024"
ALFA_SEG   = 0.50
OUTPUT_DIR = Path("output/segformer_b0")
...
def cargar_modelo():
    """
    SegFormer-B2 Cityscapes — representa SegFormer-B0 en precisión.
    ~15MB, descarga automática de HuggingFace.

    PARA REEMPLAZAR CON SegFormer-B0 REAL:
...
def procesar_video(modelo,proc,fuente,monitor):
    ...
    writer=cv2.VideoWriter(str(OUTPUT_DIR/f"{nom}_segformer_b0.mp4"),
    ...
    print(f"  Video : {OUTPUT_DIR/f'{nom}_segformer_b0.mp4'}")
